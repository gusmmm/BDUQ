...
def test_get_session_yields_session(monkeypatch, tmp_path) -> None:
    ...
    try:
        ...
        # In SQLAlchemy 2.0, binding via `with Session(engine)`
        # sets session.bind
        ...
        # Ensure we release SQLite resources to avoid ResourceWarnings
        table_registry.metadata.drop_all(temp_engine)
        temp_engine.dispose()
